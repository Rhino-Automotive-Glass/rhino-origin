...
export function ProductCodeSearch({
  value,
  onChange,
  placeholder = "Ej: FW04491",
}: ProductCodeSearchProps) {
  ...
  useEffect(() => {
    // Don't search if user hasn't typed yet (e.g., value loaded from localStorage)
    // or if user just selected a value from the dropdown
    ...
    const timeoutId = setTimeout(async () => {
      ...
      try {
        const response = await fetch(
          `/api/products/search?q=${encodeURIComponent(value)}&limit=20`,
